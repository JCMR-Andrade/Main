...
#         index='ignore', columns=["Fundo"], values=["VL_QUOTA", "VL_PATRIM_LIQ"]
#     )
...
#     df_final = pd.merge(df_classe, df_quota, on=['DT_COMPTC', 'Fundo'])
#     df_final = pd.merge(df_final, df_cotst, on=['DT_COMPTC', 'Fundo'])
#     df_final = pd.merge(df_final, df_patrim, on=['DT_COMPTC', 'Fundo'])
#     df_final = pd.merge(df_final, rentabilidade_fundos_total,on=['DT_COMPTC', 'Fundo'])
#     df_final = pd.merge(df_final, rentabilidade_media_anualizada, on=['DT_COMPTC', 'Fundo'])
#     df_final = pd.merge(df_final, retorno_periodo_anualizado, oon=['DT_COMPTC', 'Fundo'])
...
# Mostrando os nomes das colunas do dataframe copia


# Agora calcula a rentabilidade dos fundos extraídos 
df_1 = comp.calcula_rentabilidade_fundos(df)
...
# df_11 = df.assign(
#     Fundos = tuple(df["Fundo"].str.split(" 051").str[0]), # Pegando o primeiro elemento da lista
#     RentDiaria = tuple(df["CNPJ - Nome"].str.split(" // ").str[-1])
#     Fundo = tuple(df["CNPJ - Nome"].str.split(" // ").str[-1]) # Pegando o último elemento da lista
# )
...
df_10.to_csv(Arquivo, sep=';',decimal=',',index='ignore',encoding="latin1")

print(pd.DataFrame(df_1.info()))

# Acessando o sub-dataframe dentro da tupla
df_2 = pd.DataFrame(df_1.loc[:, ['DT_COMPTC',
                'Fundo',
                'CLASSE',
                'VL_QUOTA',
                'NR_COTST',
