...
def rent_fundos(
    ...
    df_classe = copia[['DT_COMPTC', 'Fundo', 'CLASSE']]
    ...
    fundo_acoes_filtrado_transformed = copia.pivot_table(
        index="DT_COMPTC", columns="Fundo", values=["VL_QUOTA", "VL_PATRIM_LIQ"]
    )
...
#Calcula a rentabilidade dos fundos
...
df = pd.DataFrame(df_1.loc[:, ['DT_COMPTC',
                'Fundo',
                'CLASSE',
                'VL_QUOTA',
                'NR_COTST',
